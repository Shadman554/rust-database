...
router.get("/instruments", async (req, res): Promise<void> => {
  ...
  const [totalResult] = await db.select({ count: count() }).from(instrumentsTable).where(where);
  const items = await db.select().from(instrumentsTable).where(where).limit(limit).offset(offset).orderBy(instrumentsTable.name);
