...
router.get("/drugs", async (req, res): Promise<void> => {
  ...
  const [totalResult] = await db.select({ count: count() }).from(drugsTable).where(where);
  const items = await db.select().from(drugsTable).where(where).limit(limit).offset(offset).orderBy(drugsTable.name);
