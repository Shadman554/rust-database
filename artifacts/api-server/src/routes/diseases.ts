...
router.get("/diseases", async (req, res): Promise<void> => {
  ...
  const [totalResult] = await db.select({ count: count() }).from(diseasesTable).where(where);
  const items = await db.select().from(diseasesTable).where(where).limit(limit).offset(offset).orderBy(diseasesTable.name);
